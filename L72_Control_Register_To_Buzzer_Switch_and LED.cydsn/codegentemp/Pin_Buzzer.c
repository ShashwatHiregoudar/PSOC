...
#include "Pin_Buzzer.h"
    ...
    #define Pin_Buzzer_SetP4PinDriveMode(shift, mode)  \
    do { \
        Pin_Buzzer_PC =   (Pin_Buzzer_PC & \
                                (uint32)(~(uint32)(Pin_Buzzer_DRIVE_MODE_IND_MASK << \
                                (Pin_Buzzer_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Pin_Buzzer_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Pin_Buzzer_SetP4PinDriveMode(shift, mode)  \
        do { \
            Pin_Buzzer_USBIO_CTRL_REG = (Pin_Buzzer_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Pin_Buzzer_DRIVE_MODE_IND_MASK << \
                                    (Pin_Buzzer_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Pin_Buzzer_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Pin_Buzzer_SetP4PinDriveMode(Pin_Buzzer__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Pin_Buzzer_DR & (uint8)(~Pin_Buzzer_MASK));
    drVal = (drVal | ((uint8)(value << Pin_Buzzer_SHIFT) & Pin_Buzzer_MASK));
    Pin_Buzzer_DR = (uint32)drVal;
}
...
{
    return (uint8)((Pin_Buzzer_PS & Pin_Buzzer_MASK) >> Pin_Buzzer_SHIFT);
}
...
{
    return (uint8)((Pin_Buzzer_DR & Pin_Buzzer_MASK) >> Pin_Buzzer_SHIFT);
}
...
void Pin_Buzzer_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Pin_Buzzer_INTCFG & (uint32)(~(uint32)position);
    Pin_Buzzer_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Pin_Buzzer_INTSTAT & Pin_Buzzer_MASK);
	Pin_Buzzer_INTSTAT = maskedStatus;
    return maskedStatus >> Pin_Buzzer_SHIFT;
}
